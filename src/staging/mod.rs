...
pub trait Staging<L> {
    ...
    fn load_inode_from_segment(&self, buf: &mut [u8], segid: SegmentId) -> impl Future<Output = Result<Option<OnDiskState>>>;
    fn load_segment_timestamp(&self, segid: SegmentId) -> impl Future<Output = Result<(i64, i64)>>;
    ...
    /// object its checkpoint's consistency point writes. For a partial segment, which
